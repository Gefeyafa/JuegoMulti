        ...
        switch (state) {
            case "idle":
                ...
                if (path_index != -1) { // Verifica que el path esté activo
                    var next_position = path_position + move_speed / path_get_length(path_enemyTutorial); // Posición siguiente en el path
                    if (next_position >= 1) {
                        next_position = 0;
                    }
                    var next_x = path_get_x(path_enemyTutorial, next_position);
                    var next_y = path_get_y(path_enemyTutorial, next_position);
            ...
            case "alert":
                ...
                if (player_distance >= sightMAX || abs(angle_diff) >= 90 || collision_line(x, y, target_player.x, target_player.y, obj_wall, true, true) != noone) {
                    var start_x = path_get_x(path_enemyTutorial, 0);
                    var start_y = path_get_y(path_enemyTutorial, 0);
                    ...
                    if (distance_to_start > move_speed) {
                        ...
                        state = "idle";
                        path_start(path_enemyTutorial, move_speed, path_action_continue, false);
                    }
                } else {
